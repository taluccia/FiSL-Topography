...
```{r}
( ss1_filter = field %>% 
  filter(ecoregion_name_l2 =="Softwood Shield") %>%
  filter(longitude > -90)
...
```{r}
cat(paste0(sprintf('"%s"', ss1_filter$project_id), collapse = ", "))
```
...
```{r}
cat(paste0(sprintf('"%s"', ss2_filter$project_id), collapse = ", "))
```
## SSBP1
```{r}
( ssbp1_filter = field %>% 
  filter(ecoregion_name_l2 %in% c("Softwood Shield", "Boreal Plains")) %>%
  filter(latitude > 56) %>%
    filter(longitude < -105)
  
)
```

```{r}
cat(paste0(sprintf('"%s"', ssbp1_filter$project_id), collapse = ", "))
```
...
( ssbp2_filter = field %>% 
  filter(ecoregion_name_l2 %in% c("Softwood Shield", "Boreal Plains")) %>%
  filter(latitude < 56) %>%
    filter(longitude < -102)
)
```

```{r}
cat(paste0(sprintf('"%s"', ssbp2_filter$project_id), collapse = ", "))
```
...
```{r}
cat(paste0(sprintf('"%s"', tp2_filter$project_id), collapse = ", "))
```
...
```{r}
cat(paste0(sprintf('"%s"', unique(tp3_filter$project_id)), collapse = ", "))
```
...
( field1 = field %>% 
    ...
    mutate(group2 = ifelse(project_id %in% ss1, "ss1",
                    ifelse(project_id %in% ss2, "ss2",
                    ifelse(project_id %in% ssbp1, "ssbp1",
                    ifelse(project_id %in% ssbp2, "ssbp2",
                           ifelse(project_id %in% ts1, "ts1",
                                  ifelse(project_id %in% tstp1, "tstp1",
                                         ifelse(project_id %in% tp1, "tp1",
                                                ifelse(project_id %in% tp2, "tp2",
                    ifelse(project_id %in% tp3, "tp3", "NA")))))))))) %>%
    mutate(moistCl = ifelse(moisture==1, "xeric", 
...
```{r}
field1 %>% filter(group1 == "SK")
```

```{r}
field1 %>% filter(group2 == "tp2")
```
...
(
  all_field = field1 %>%
    dplyr::select(project_id:longitude,group1, group2, moisture) %>%
